...
class DB {
  ...
  #createDatabase() {
    ...
    db.serialize(() => {
      db
        ?.run(
          'CREATE TABLE "Banned" ("ID" INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE, "Time" TEXT NOT NULL, "BannedBy" NUMERIC NOT NULL, "ScreenName" TEXT NOT NULL, "IPAddress" TEXT NOT NULL, "Reason" TEXT NOT NULL);'
        )
        .run(
          'CREATE TABLE "Feedbag" ("PID" INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE, "ID" INTEGER NOT NULL, "Name" TEXT NOT NULL, "GroupID" INTEGER NOT NULL, "BuddyID" INTEGER NOT NULL, "ClassID" INTEGER NOT NULL, "Attributes" BLOB);'
        )
        .run(
        ...
        .run(
          'CREATE TABLE "RateLimitMapping" ("ID" INTEGER PRIMARY KEY AUTOINCREMENT UNIQUE, "RateClass" INTEGER NOT NULL, "SNACFamily" INTEGER NOT NULL, "SNACSubType" INTEGER NOT NULL);'
        );
    });
    db.serialize(() => {
      db
        ?.run(
          'INSERT INTO "RateClasses" (WindowSize, ClearThreshold, AlertThreshold, LimitThreshold, MinAverage, MaxAverage) VALUES (80, 2500, 2000, 1500, 800, 6000);'
        )
        .run(
          'INSERT INTO "RateClasses" (WindowSize, ClearThreshold, AlertThreshold, LimitThreshold, MinAverage, MaxAverage) VALUES (80, 3000, 2000, 1500, 1000, 6000);'
        )
        .run(
          'INSERT INTO "RateClasses" (WindowSize, ClearThreshold, AlertThreshold, LimitThreshold, MinAverage, MaxAverage) VALUES (20, 5100, 5000, 4000, 3000, 6000);'
        )
        .run(
          'INSERT INTO "RateClasses" (WindowSize, ClearThreshold, AlertThreshold, LimitThreshold, MinAverage, MaxAverage) VALUES (20, 5500, 5300, 4200, 3000, 8000);'
        )
        .run(
          'INSERT INTO "RateClasses" (WindowSize, ClearThreshold, AlertThreshold, LimitThreshold, MinAverage, MaxAverage) VALUES (10, 5500, 5300, 4200, 3000, 8000);'
        );
    });
    db.serialize(() => {
      db
        ?.run('INSERT INTO RateLimitMapping (RateClass, SNACFamily, SNACSubType) VALUES (1, 1, 1);')
  ...
  getBuddyList(id: number): Promise<BuddyList[]> {
    return new Promise((resolve) => {
      const rows: BuddyList[] = [];
      db?.each(
        'SELECT * FROM Feedbag WHERE [ID] = ? ORDER BY GroupID, BuddyID',
        id,
        (err, row: BuddyList) => {
          rows.push({
            PID: row.PID,
        ...
        () => {
          if (!rows.length) {
            const timestamp = Util.Dates.GetTimestamp();
            db?.serialize(() => {
  ...
  getOnBuddyList(screenName: string): Promise<number[]> {
    return new Promise((resolve) => {
      const rows: number[] = [];
      db?.each(
        'SELECT * FROM Feedbag WHERE Name = ? AND ClassID = 0',
        screenName,
        (err, row: BuddyList) => {
          rows.push(row.ID);
        },
  ...
  getFeedbagTimestamp(id: number): Promise<Date> {
    return new Promise((resolve) => {
      db?.get('SELECT FeedbagTimestamp FROM Memberships WHERE ID = ?', id, (err, row: User) => {
        resolve(row.FeedbagTimestamp);
      });
    });
  }
  updateAdminInfo(id: number, formattedScreenName: string, emailAddress: string) {
    return new Promise((resolve) => {
      db?.serialize(() => {
        db?.run(
          'UPDATE Memberships SET FormattedScreenName = ?, EmailAddress = ? WHERE ID = ?',
          formattedScreenName,
          emailAddress,
          id,
          () => {
  ...
  updateFeedbagItem(id, name, groupId, itemId, classId, attributes: Buffer) {
    return new Promise((resolve) => {
      db?.serialize(() => {
        db
          ?.run(
            'UPDATE Feedbag SET Name = ?, GroupID = ?, ClassID = ?, Attributes = ? WHERE ID = ? AND GroupID = ? AND BuddyID = ? AND ClassID = ?',
            name,
            groupId,
            classId,
            attributes.length === 0 ? '' : attributes,
          ...
          .get(
            'SELECT * FROM Feedbag WHERE ID = ? AND GroupID = ? AND BuddyID = ? AND ClassID = ?',
            id,
            groupId,
            itemId,
            classId,
  ...
  deleteFeedbagItem(id: number, name: string, groupId: number, itemId: number, classId: number, attributes: Buffer) {
    console.log('db.delete', id, name, groupId, itemId, classId, attributes);
    return new Promise((resolve) => {
      db?.run(
        'DELETE FROM Feedbag WHERE ID = ? AND GroupID = ? AND BuddyID = ? AND ClassID = ?',
        id,
        groupId,
        itemId,
        classId,
  ...
  updateFeedbagMeta(id: number) {
    return new Promise((resolve) => {
      const timestamp = Util.Dates.GetTimestamp() + 300;
      let count = 0;
      db?.get('SELECT COUNT(*) FeedbagItems FROM Feedbag WHERE ID = ?', id, (err, row: User) => {
        count = row.FeedbagItems;
        db?.serialize(function () {
          db?.run(
            'UPDATE Memberships SET FeedbagTimestamp = ?, FeedbagItems = ? WHERE ID = ?',
            timestamp,
            count,
            id,
            () => {
